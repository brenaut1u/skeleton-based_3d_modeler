    ...
    public:
        ...
        void add_sphere_at_pos(int screen_pos_x, int screen_pos_y) {
            ...
            if (std::get<0>(find_sphere) != -1) {
                sphere new_sphere = sphere(std::get<1>(find_sphere), 0.3, std::get<2>(find_sphere));
                spheres_group.add_sphere(new_sphere, std::get<0>(find_sphere));
                world->add(std::make_shared<sphere>(std::get<1>(find_sphere), 0.3, std::get<2>(find_sphere)));
            }
        }
    private:
